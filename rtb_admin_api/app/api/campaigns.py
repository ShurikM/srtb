...
def list_campaigns(session: str = Cookie(None)):
    user = get_logged_user(session)
    return list(campaign_store.values())

# === REAL DB ACCESS ===
@router.get("/active", response_model=list[CampaignRuntime])
def get_active_campaigns(db: Session = Depends(get_db), session: str = Cookie(None)):
    user = get_logged_user(session)
    now = datetime.utcnow()
...
def read_campaigns(skip: int = 0, limit: int = 10, db: Session = Depends(get_db), session: str = Cookie(None)):
    user = get_logged_user(session)
    return db.query(models.Campaign).offset(skip).limit(limit).all()

@router.post("/", response_model=schemas.CampaignRead)
def create_campaign(campaign: schemas.CampaignCreate, db: Session = Depends(get_db), session: str = Cookie(None)):
    user = get_logged_user(session)
...
def update_campaign(campaign_id: int, campaign_data: schemas.CampaignCreate, db: Session = Depends(get_db), session: str = Cookie(None)):
    user = get_logged_user(session)
...
def delete_campaign(campaign_id: int, db: Session = Depends(get_db), session: str = Cookie(None)):
    user = get_logged_user(session)
    campaign = db.query(models.Campaign).filter(models.Campaign.id == campaign_id).first()
    if campaign is None:
        raise HTTPException(status_code=404, detail="Campaign not found")
...
def read_campaign(campaign_id: str, db: Session = Depends(get_db), session: str = Cookie(None)):
    user = get_logged_user(session)
